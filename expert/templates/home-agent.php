...
<div class="expert-local-ai" data-expert-local-ai><div><strong><?php echo esc_html( sprintf( __( '%s is getting ready', 'expert' ), $expert_agent['name'] ) ); ?></strong><p><?php esc_html_e( 'Questions, curiosity and fresh evidence help this Agent’s understanding grow over time.', 'expert' ); ?></p></div><progress class="expert-local-progress" max="100" value="0" aria-label="<?php esc_attr_e( 'Agent preparation progress', 'expert' ); ?>"></progress><p class="expert-local-status" role="status" aria-live="polite"><?php esc_html_e( 'Waking up…', 'expert' ); ?></p></div>
